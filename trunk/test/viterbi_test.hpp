...
class HmmLineMatchTest {
 public:
  void test(string wkt_file) {
    ...
    bg::read_wkt(wkt, line_string);

    limap::proj::ProjTransform<LineString> tr(21);
    LineString proj_line_string;
    tr.execute(proj_line_string, line_string);
    for (auto pt : proj_line_string) {
      cout << pt.get<0>() << ", " << pt.get<1>() << std::endl;
    }

    LineString input_line_string;
    for (auto pt : proj_line_string) {
      input_line_string.push_back(Point2D(pt.get<0>(), pt.get<1>()));
    }
